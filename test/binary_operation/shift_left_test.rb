...
class BinaryShiftLeftTest < Javascript::Test
  ...
  def test_shifting_a_number_bigger_than_32_bits
    ...
    assert_equal 1891627008, evaluate("(3 ** 39) << 1")
    assert_equal -1891627008, evaluate("-(3 ** 39) << 1")
  end

  def test_shifting_by_a_negative_number
    assert_equal 0b1000000000000000000000000000000, evaluate(%(0b1001 << -2))
    assert_equal 0b1000000000000000000000000000000, evaluate(%(0b10101001 << -2))
    assert_equal -0b1000000000000000000000000000000, evaluate(%(-0b10101001 << -2))
  end
end
